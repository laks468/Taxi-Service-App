...
class HelperMethods{
  ...
  static int estimateFares (DirectionDetails details, int durationValue){
    ...
    ///per minute = 2Rs
    ///base fare = 25 Rs
    /// Total Fares = Base Fare + Distance Fare + Time Far
  ...
  static void getHistoryInfo (context){
    ...
    DatabaseReference earningRef = FirebaseDatabase.instance.reference().child('drivers/${FirebaseAuth.instance.currentUser.uid}/earnings');
    ...
    DatabaseReference historyRef = FirebaseDatabase.instance.reference().child('drivers/${FirebaseAuth.instance.currentUser.uid}/history');
